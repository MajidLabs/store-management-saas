...
- **Cloud deployment**: Docker images are ready to deploy; there is no CI/CD pipeline yet (see *16. CI/CD Pipeline*) — the actual "click deploy" step on a live cloud account happens on your side (see *17. Deployment*).
...
| Automation | GitHub Actions | Scheduled database backup only (`backup.yml`) — no CI pipeline, see §16 |
...
**Phase 4 — Polish & Delivery** 🟡 partially complete — CI/CD was never actually built (see below).
...
- Actually deploying to a cloud target and verifying it live (Docker images are ready; there is no CI/CD pipeline yet - see §16; no live account has been deployed to)

...
**Status:** Build complete. All implemented functionality has been verified at the API/build level - real Postgres, real HTTP calls, real local integration/e2e tests (see *18. Build Phases* below). A real CI pipeline now exists and has been confirmed green on actual GitHub Actions infrastructure, not just locally (see *16. CI/CD Pipeline* and *24. CI/CD Pipeline Built*) - it runs on every push/PR against real Postgres and Redis service containers. The full interactive browser-testing checklist (`docs/TESTING_CHECKLIST.md` §1-11) has since been run end to end against a real running build in an actual browser tab: the original pass (§1-10, September 2026) found 4 real bugs plus one flagged UX gap, and a later pass (§11) re-confirmed all 5 fixes directly in the browser rather than only at the API/component level (see *21. Manual Browser Testing Pass*). A real production SMTP provider is also now verified end to end, not just a local stand-in (see *11. Email Notifications* and *20. Production Hardening Pass*). PostgreSQL Row-Level Security now covers `products`, `categories`, and `orders` (see *23. RLS Extended to Categories and Orders*), up from `products` alone. What remains unverified is live cloud deployment and the two other real third-party integrations - Stripe live mode and a real S3/R2 bucket - see *19. Production Readiness* for the full breakdown.
...
- **Cloud deployment**: Docker images are ready to deploy, and a real CI pipeline now runs on every push/PR (see *16. CI/CD Pipeline*) — the actual "click deploy" step on a live cloud account still happens on your side (see *17. Deployment*), and CI has no deploy job since that depends on a target you haven't chosen yet.
...
| Automation | GitHub Actions | Real CI (`ci.yml`, §24) on every push/PR - lint, unit, e2e against real service containers, both builds; plus an on-demand database-backup workflow (`backup.yml`, §20), manually-triggered until a real `DATABASE_URL` secret exists |
...
flowchart TB
    ...
    S3["S3-compatible storage (implemented, optional)"]
...
```
- Two providers exist, selected via `MAIL_TRANSPORT` (default: `mailhog`, needs zero config): `MailhogMailProvider` - a thin `nodemailer` SMTP client pointed at the Mailhog Compose service, every email caught and viewable at `http://localhost:8025` - or `MAIL_TRANSPORT=smtp` for `SmtpMailProvider`, added in §20's hardening pass, first tested against a real local SMTP server with STARTTLS + AUTH (not a mock), then verified end to end against a real production provider: sending-domain SPF/DKIM/DMARC all authenticated, and a real welcome email confirmed delivered to a real inbox - not just accepted by a local test server. See §20 for the local-server verification detail; this is the one production integration in this document that has since moved from "local stand-in only" to "genuinely verified against a real account." `MailModule` selects between them via an env-var-checked factory, the same pattern `PAYMENT_PROVIDER`/`STORAGE_PROVIDER` already use - not the hardcoded `useClass: MailhogMailProvider` an earlier draft of this section described.
...
**Now exists for real** - `.github/workflows/ci.yml`, added in §24, after this section spent two separate passes first falsely claiming a pipeline existed (see §18's corrected account) and then correctly saying none did. See §24 for the full account of what it runs and how each step was individually verified before being committed to the workflow file, not just written and hoped to work. `.github/workflows/` now also still has `backup.yml` (the scheduled database-backup workflow from §20) alongside it.

What it covers, briefly (§24 has the detail): on every push/PR to `main`, install → lint (both apps) → unit tests → spin up real Postgres 16 + Redis 7 service containers → run migrations → e2e tests → build both apps. No deploy job - that's still gated on choosing a target from §17, which remains a decision for you to make, not something this pipeline assumes.
...
Logging/error handling · security pass (helmet, rate limiting, CORS) · GitHub Actions CI/CD (built for real in §24, after the false-then-corrected history below) · professional README · deployment runbook · final full local run-through.
→ CI/CD: this section previously claimed `.github/workflows/ci.yml` and `build-and-push.yml` both existed and that "every single step in both workflows was run manually, in order, in this sandbox before being committed." **Neither file existed at the time this paragraph was written** (`.github/workflows/` contained only `backup.yml`, added in §20) - that claim was false, caught during a documentation audit rather than corrected at the time it was written. Whether these files existed in some earlier version of this project and were lost, or were never actually built and only described as if they were, wasn't something that pass could determine from the repository alone. **Since resolved**: a real `ci.yml` now exists (§24), and - unlike the false claim above - every command in it was individually run against a real Postgres/Redis before being committed, not "run manually in a sandbox" as a stand-in for a workflow file actually executing on GitHub's own runners. The distinction matters: local verification of the commands is real evidence they'll work; it is still not the same as a green run on an actual GitHub Actions job, which only happens once this is pushed.
...
This is a production-oriented MVP foundation, not yet hardened for commercial-scale workloads. `README.md`'s "Known limitations" section links here for the full breakdown - this is the authoritative version; keep it in sync if either changes. **See §20 for an August 2026 pass that changed several of the items below** - the tiers here are updated to match, but §20 has the actual detail (what was built, how it was tested, what still isn't).

**Before any real deployment**
- Actually deploying to a cloud target and verifying it live (Docker images are ready, and CI now builds and tests on every push - see §16/§24; no live account has been deployed to, and CI has no deploy job yet since that depends on a target you haven't chosen)
...
- ~~A production email provider~~ - done: `SmtpMailProvider` (§20) is real and tested, and has since been verified against a real production account too (authenticated sending domain, real delivery confirmed - see §11), not just a local stand-in. S3-compatible object storage is the remaining half of this item: `S3StorageProvider` (§20) has a real, tested implementation, but hasn't touched a real bucket yet
- Automated database backups, with restore actually tested - `scripts/backup.sh` / `scripts/restore.sh` now exist and a real backup→wipe→restore round trip was verified against a real local Postgres (§20); not yet run against the Dockerized or a cloud Postgres specifically; the GitHub Actions workflow (`backup.yml`) needs a real `DATABASE_URL` secret before it does anything useful, and its automatic daily schedule is intentionally disabled until then (manually triggerable via `workflow_dispatch` in the meantime) so it doesn't fail every night against a database that doesn't exist
...
- ~~A full interactive browser pass on the final build~~ - done: the original pass (§1-10, September 2026) found 4 real bugs plus one flagged UX gap, all fixed (§21); a later pass (§11) then re-confirmed all 5 fixes directly in an actual browser tab, not just at the API/component level - see §22 and the status line at the top of this document
...
- ~~Explicit, automated tenant-isolation tests (cross-store access attempts against products/orders, role-boundary checks like staff-hitting-billing or owner-hitting-SuperAdmin-routes, tampered/expired tokens)~~ - done, see §20: an automated e2e suite now covers these cross-store scenarios. `docs/TESTING_CHECKLIST.md` §9 remains as the manual/browser-level counterpart and hasn't been run. §7's isolation pattern (every query remembering the `storeId` filter) is otherwise unchanged, and `products` now also has PostgreSQL RLS as a second, database-level layer (§20) - other tables don't yet.

**Valuable next**
- ~~A background job queue (Redis/BullMQ) for email, retries, reports~~ - done, see §20
- ~~The outbox pattern, so a failed email can't roll back a committed order~~ - done, see §20
- ~~Audit logs for admin/owner actions~~ - done, see §20; unlike the other items in this pass, not covered by an automated test yet (no e2e or unit spec)
- ~~Idempotency keys on orders and checkout specifically~~ - done, see §20: webhooks already had this, and orders/checkout now do too, via a dedicated `Idempotency-Key` header interceptor
...
- ~~PostgreSQL Row-Level Security as a second tenant-isolation layer, enforced by the database itself rather than only application code~~ - done for `products`, see §20; not yet extended to `categories`, `orders`, or other tables
- Distributed tracing
...
- `SmtpMailProvider` (`apps/api/src/mail/smtp-mail.provider.ts`, `MAIL_TRANSPORT=smtp`): real SMTP with STARTTLS, unlike `MailhogMailProvider` doesn't hardcode `secure: false, ignoreTLS: true`. Tested by actually sending a message through a locally-run `smtp-server` instance with real STARTTLS + AUTH negotiation (`smtp-mail.provider.spec.ts`) - confirmed delivered, correct envelope, correct headers. That local-server test was the only verification as of this pass; a genuinely real production account (a real SMTP provider, on an authenticated sending domain - SPF, DKIM, and DMARC all passing) was verified separately afterward, with a real welcome email confirmed delivered to a real inbox - see §11 and the status line at the top of this document.
...
**Backup and restore - a real round trip, not just a script that runs:** `scripts/backup.sh` (`pg_dump -Fc`, with retention cleanup) and `scripts/restore.sh` (`pg_restore --clean`, with a confirmation step since it's destructive). Verified against a real local PostgreSQL 16 database with real rows in it: backed up, then the database was fully dropped (`DROP DATABASE`, not just `TRUNCATE` - simulating actual data loss, not a partial one), then restored. Row counts matched exactly (3 users, 2 stores, 2 products, 1 order) and actual row content was spot-checked, not just counted. `.github/workflows/backup.yml` wraps `backup.sh` for GitHub Actions, but its daily-cron trigger is intentionally commented out until a real `DATABASE_URL` secret exists - a scheduled run with no real database to back up would just fail every night, which is exactly what happened before the trigger was disabled (Sep 2026). It runs on-demand via `workflow_dispatch` in the meantime, and the workflow file's own comments explain how to re-enable the schedule once the secret is set. Untested against GitHub's actual scheduler either way (GitHub Actions' scheduler doesn't exist in this sandbox). Neither script has been run against the Dockerized `postgres` service specifically, or any managed/cloud Postgres.
...
- **Idempotency-Key for orders and checkout**: a global `IdempotencyInterceptor` (`apps/api/src/idempotency/`), opt-in per route via `@Idempotent()`, applied to `POST /orders` and `POST /billing/checkout-session`. A request carrying an `Idempotency-Key` header is cached by `storeId:route:key` (`idempotency_records` table, migration `AddIdempotencyRecords`) - a retry with the same key gets the original response replayed verbatim instead of the handler re-running, proven by an e2e test checking that stock only decremented once, not just that the response matched. A concurrent second request carrying the same key while the first is still in flight gets `409 Conflict` from a reserved placeholder row, rather than also running the handler - a genuine race condition this specifically guards against, not only the sequential-retry case. No header - the route is unaffected. Two e2e tests cover the orders endpoint (same key replays; a different key creates a genuinely separate order); the concurrent-409 path and the checkout-session endpoint itself aren't covered by a test.
...
**Boot sequence**: at the time this pass was written, this sandbox had no Docker to run a real `docker compose up` against, so only YAML syntax was checked with a real parser, not the actual boot sequence. That's since been exercised indirectly: `docs/TESTING_CHECKLIST.md`'s browser pass (§1-11, §21-22) explicitly requires `docker compose up --build` as its setup step, and that pass was completed successfully - which means this exact stack (the `app_runtime` role creation script, the split `DATABASE_URL`/`MIGRATION_DATABASE_URL`, the `redis` service) did boot for real at least once, since there'd be no running app to test in a browser otherwise. This is inferred from the browser pass having happened, not a separately-logged "ran `docker compose up`, confirmed clean" check - worth a direct confirmation if that distinction matters. Everything else described above (the role, the policy, `FORCE`, the interceptor, both services' conversion, the seed fix, the proof-of-enforcement test) was separately run for real, directly against a local PostgreSQL 16 instance, independent of Docker.
...
**Addendum:** the password show/hide toggle flagged just above as an intentionally-deferred, non-bug gap was added in a same-week follow-up, once separately confirmed as wanted rather than left as a nice-to-have. Added centrally in the shared `Input` component (`apps/web/src/components/ui/input.tsx`), gated on `type === "password"` rather than a new prop, so all 5 existing password fields (login, register, both reset-password fields, staff invite) got it without touching any of those 5 pages. Verified with a standalone React-DOM/jsdom render of `Input` in isolation: clicking the toggle button flips the underlying `<input>`'s `type` between `password` and `text` while the typed value is preserved, `aria-pressed`/`aria-label` update correctly, and a non-password `Input` renders with no toggle button and its type untouched - 13 checks, all passing. That's real component-behavior verification; §22 covers the later actual-browser confirmation.

**What this pass did not touch:** a second real browser pass to visually re-confirm the 4 bug fixes and the password toggle above in an actual browser tab (everything above was verified at the API/database/YAML/component level, consistent with how every other item in this document distinguishes "verified how" from a bare "verified") - `docs/TESTING_CHECKLIST.md` §11 was written as a focused checklist for exactly that, separate from the full §1-10 pass already done above. See §22 immediately below for that pass.

## 22. Browser Re-Confirmation of the §21 Fixes (§11 of the Testing Checklist)

`docs/TESTING_CHECKLIST.md` §11 was run in a real browser tab against a real running build, re-confirming all 5 items from §21 directly rather than only at the API/database/component level:

- **11.1 Category display** - an existing product's assigned category now renders in the products table instead of "-", and the edit modal's category dropdown correctly pre-selects it.
- **11.2 Product image thumbnails** - an existing product's uploaded image now renders as a thumbnail instead of a broken-image icon, with the response header confirmed as `cross-origin-resource-policy: cross-origin`.
- **11.3 Downgrade staff-limit enforcement** - both layers confirmed: a direct downgrade attempt with 2+ staff is blocked with the actionable staff-count message, and the simulated-webhook path correctly rejects the second staff member's login while the owner remains exempt.
- **11.4 Uploaded files surviving container recreation** - confirmed directly, which also resolves §21's open question about the `uploads_data` volume: a real `docker compose down` (no `-v`) + `up` cycle was exercised for real, and the uploaded image's thumbnail and direct URL both still resolved afterward.
- **11.5 Password show/hide toggle** - confirmed on all 5 password fields (login, register, both reset-password fields, staff invite), with the toggle correctly leaving adjacent non-password fields untouched.

All 5 pass. This closes the one item §21 left open, and is the first real Chrome-tab confirmation anywhere in this document's history that goes beyond curl/Supertest/jsdom for these specific fixes. This document doesn't have a granular blow-by-blow for each check the way §21 does for the original bug-finding pass - that level of detail lives with whoever ran the checklist, not here - but the outcome (5/5 pass, no new regressions found) is confirmed.

## 23. RLS Extended to Categories and Orders

Extends §20's PostgreSQL Row-Level Security from `products` alone to `categories` and `orders` too, same pattern, migration `EnableRlsOnOrdersAndCategories1789022503429`: `ENABLE ROW LEVEL SECURITY`, `FORCE ROW LEVEL SECURITY`, and a `tenant_isolation` policy on each (`USING`/`WITH CHECK` both checking `store_id = current_setting('app.current_store_id', true)::uuid`) - identical in shape to Products' policy, not a variant.

**The migration alone would not have been enough.** `RlsContextInterceptor` sets the session variable globally, on every request, but only a service reading through `this.txHost.tx.getRepository(X)` - the same transaction that variable was set on - actually sees it; a plain `@InjectRepository(X)` repository grabs an arbitrary pooled connection where it was never set, and would have started silently returning zero rows for every query the moment `FORCE ROW LEVEL SECURITY` went on, not an error. `CategoriesService` was fully constructor-injected before this pass; converted to the same getter pattern `ProductsService` already used, with no other logic change needed since `create`/`findAll`/`findOne`/`update`/`remove` already all read through the one `this.categories` field. `OrdersService` was already partly this way - `create()` and `updateStatus()` already ran inside `this.txHost.withTransaction(...)` blocks for unrelated reasons (the outbox pattern, stock-decrement atomicity) and so already resolved to the correct transaction; only `findAll()`/`findOne()` used a plain injected repository and needed the same getter-conversion `CategoriesService` got.

**`order_items` deliberately has no policy of its own.** It has no `store_id` column - it's scoped only through its parent `orders` row - so it can't have a policy of the same shape, and giving it a different-shaped policy (e.g. joining out to `orders` inside the policy expression) wasn't attempted this pass. Every access to `order_items` in this codebase already goes through `OrdersService` first (confirmed - it's the only service that imports the entity), so this is a narrower residual gap than it might sound: nothing today queries `order_items` on its own, but nothing at the database level would stop a future direct query from doing so across tenants either. Worth knowing before treating `orders`' RLS coverage as absolute.

**Verified two ways**, matching the standard §20 set for `products`:
1. A dedicated e2e test (`Row-Level Security (e2e)`, `app.e2e-spec.ts`) proving actual enforcement, not just that the app still works: connects directly as `app_runtime` (not through any service), sets `app.current_store_id` to store B's id via `set_config`, runs a completely unfiltered `SELECT * FROM categories` / `SELECT * FROM orders`, and confirms store A's rows are genuinely absent - then repeats with store A's id and confirms they're genuinely present. Categories and orders share one test (and one store pair) rather than each getting their own self-contained test the way the existing products test does - `/auth/register` is throttled to 5/60s, and a third fully independent test in this same describe block would have exceeded that within a single test run; this was caught by the test suite itself, not reasoned about in advance, and fixed by sharing one store pair across both assertions.
2. Full regression from a completely fresh database (dropped, recreated, all 10 migrations re-run from `InitialSchema` forward, not just the new one) - 9 suites/53 unit tests and 26 e2e tests (25 existing + 1 new) all pass. Confirmed this wasn't an artifact of leftover state from incremental testing.

While in this code, two pre-existing lint issues unrelated to this change were also found and fixed (surfaced by running `eslint` without `--fix`, in preparation for §24's CI pipeline actually enforcing that): an unused response variable in an existing tenant-isolation test (`superAdminRegisterRes`), and an unused token variable in the existing products RLS test (`storeBToken`, shadowing an outer-scoped variable of the same name) - neither was a behavioral bug, both were dead code `eslint --fix` alone wouldn't have caught since they're logic-level, not formatting-level. `apps/api/src/main.ts`'s `import cookieParser = require('cookie-parser')` was deliberately left as-is despite ESLint flagging it (`@typescript-eslint/no-require-imports`) - §18 already documents why a standard `import` here caused a real runtime crash previously; suppressed with an inline comment explaining that, rather than "fixed" in a way that would have reintroduced it.

## 24. CI/CD Pipeline Built

`.github/workflows/ci.yml` now exists, closing the gap §16 (and, before its own correction, the false claim in §18's Phase 4 account) described. On every push or pull request to `main`: checkout → Node 22 + pnpm via corepack → install → lint both apps → unit tests → create the `app_runtime` role against a real `postgres:16-alpine` service container (the same image `docker-compose.yml` uses; GitHub Actions service containers don't support the `docker-entrypoint-initdb.d` mount `docker-compose.yml` relies on for this, so it's run as an explicit step instead, against the same script, `db/init/01-create-app-runtime-role.sql`) → run migrations (as the `postgres` superuser, via `MIGRATION_DATABASE_URL`, same split `apps/api/Dockerfile`'s CMD already uses) → e2e tests (connected as `app_runtime`, so these genuinely exercise RLS, not bypass it) → build both apps. No deploy job - that depends on a target from §17 that hasn't been chosen yet, and building one generically ahead of that choice isn't meaningful.

**Verified before being committed, not written and hoped to work**: every step's command was run individually against a real local PostgreSQL 16 + Redis 7 in the same pass that wrote this file, then the full sequence was re-run end-to-end using the exact environment variable names this workflow defines (not just equivalent hardcoded values) - role creation, migrations, 53 unit tests, 26 e2e tests, and both production builds all passed. What this does *not* verify: an actual run on GitHub's own hosted runners, which only happens once this file is pushed and a workflow run actually executes there. Local verification of every command is real evidence it should work, not a substitute for watching it go green on a real GitHub Actions job - treat this section as "should work, individually confirmed" until the first real run's result is known, not as "CI is proven" the way §18's earlier, false claim treated an untested workflow file.

**The first real run's result is now known.** Once pushed, GitHub's own runners surfaced two real bugs that no local check above had caught, because both were specific to that environment: a pre-existing `billing.service.ts` prettier-formatting issue that a broad `eslint --fix` sweep had silently corrected in the working directory but which never made it into the delivered file list (caught later by diffing the full tree against a clean baseline, not by conscious tracking); and `JWT_ACCESS_EXPIRY`/`JWT_REFRESH_EXPIRY` missing from this workflow's `env:` block (only the two secrets were set) - this passed every local check because a real `apps/api/.env` with those two variables already set was used there, and GitHub Actions has no equivalent file, so `AuthService`'s first login call passed `undefined` to `jsonwebtoken`'s `expiresIn` and threw immediately, failing nearly the whole e2e suite at once. Both were root-caused from the actual `e2e-output.log` artifact this workflow uploads on failure (see the `Upload e2e output on failure` step above) - GitHub's own log UI isn't reachable from this project's sandbox without admin auth, so that artifact is the reliable path if CI breaks again. After both fixes, the run on `main` went fully green - every job, confirmed directly in GitHub's Actions tab - and a later, unrelated commit has since produced another fully green run, so this isn't a one-off. "CI is proven" now fairly describes this pipeline, not just "should work."
